<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="房间状态" prop="fangjianzhuangtai">
          <el-input v-model="ruleForm.fangjianzhuangtai" 
              placeholder="房间状态" clearable  :readonly="ro.fangjianzhuangtai"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	ispay : false,
      ...
      rules: {
          yudingbianhao: [
                { required: true, message: '预订编号不能为空', trigger: 'blur' },
          ],
          fangjianhao: [
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fangjianyuding/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fangjianyudingCrossAddOrUpdateFlag = false;
                  this.parent.search();
